...
measurement in preliminary form---confirming the central churn-regime
hypothesis---giving strong feasibility evidence for the
proposed integration and measurement.
...
Preliminary five-seed experiments on the primary cohort already support the
latter: a whole-silo exit costs roughly six times the utility of transient
churn, and a count-matched control isolates the damage to \emph{who} leaves
rather than \emph{how many}.
...
\emph{Preliminary status: supported---a five-seed study on the primary cohort
already separates the regimes and confirms the isolation test
(\S\ref{sec:feas}).}
...
\emph{confirming H2 in preliminary form}. These results de-risk the utility
axis entirely; the funded work scales it (higher-capacity averageable clients,
calibration and significance testing per~\cite{tennis}), couples it to the
systems axis, and assembles the frontier.
...
Our preliminary five-method comparison (\S\ref{sec:prelim}) already shows the
aggregator choice is immaterial at $K{=}3$ short of pathological heterogeneity,
so the Year-2 robustness check shifts to model capacity: replacing the linear
averageable client with a small MLP to test whether degradation magnitudes are a
...
calibration and significance testing, extending the executed preliminary study
(\S\ref{sec:prelim}) that already confirms the H2 ``who leaves $>$ how many
leave'' finding via its count-matched isolation test; funded work adds
higher-capacity averageable clients (MLP), finer rate sweeps, and real-hospital
silo replication (\S\ref{sec:data}).
...
the word ``privacy.''
\item \textbf{Robustness (Year 2).} The aggregator dimension is already closed
in preliminary form---five methods compared, FedAvg justified
(\S\ref{sec:prelim})---so robustness shifts to model capacity (a small MLP
averageable client) and to replication on real hospital silos (eICU), testing
whether Thrust-3 degradation magnitudes are artifacts of the linear client or of
...
completed on the primary cohort (preliminary form already executed,
\S\ref{sec:prelim}); first cost--utility curves and the three-regime churn
finding---this is the
PhD dissertation's core. \textbf{Year 2:} Thrust 4 multi-project consent and the
frontier; Thrust 5 privacy (DP/secure-agg) and robustness (FedProx/SCAFFOLD).
