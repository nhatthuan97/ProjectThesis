...
many}, overturning the intuitive headcount model. Executed experiments on the
readmission task confirm that hypothesis: removing an entire positive-heavy silo
costs $-0.059$ AUROC, against $-0.039$ for the same number of patients
withdrawing at random---and a full rerun of the churn grid with a
higher-capacity neural client reproduces every delta within noise, showing the
cost structure of consent churn is a property of the problem, not of the model.
...
Status: confirmed---executed five-seed results on the primary dataset confirm H2
via a count-matched isolation test, and an executed model-capacity replication
(a one-hidden-layer MLP client) reproduces the deltas within noise
...
(Section~\ref{sec:fl_design}), and an executed model-capacity check shows the
churn-cost measurements themselves are client-independent
(Section~\ref{sec:mlp_check}). The Year-1 claim is deliberately
...
\emph{how many}. Executed five-seed experiments on the primary task demonstrate
the separation, confirm the claim via a count-matched isolation test, and show
the result is model-independent: a full rerun with a higher-capacity MLP client
reproduces every delta within one standard deviation
(Section~\ref{sec:utility_axis}). \emph{(RQ2.)}
...
Sparks offers caching intermediate data in RAM, which significantly increases performance compared to disk-based processing in MapReduce. Spark also provides support and integration with other programming languages and platforms. Spark also provided higher-level API and more expressive programming models thus lower the barrier of entry compared to MapReduce.
...
\begin{figure}[t]
\vspace{0.5em} % added for space adjustment. Modify/delete for your paper
    \centering    \includegraphics[width=1.0\linewidth]{SparkPaperFigure/Age-Dist.jpg}
...
\vspace{0.5em} % added for space adjustment. Modify/delete for your paper
    ...
    \caption{Spark's Training Time (Cervical Cancer Classification)}
    \label{Spark-Time-OG}
\end{figure}
%-////////////////////////////////////////////////////////////////////
\begin{figure}[t]
...
\begin{itemize}
  ...
  \item \textbf{Federated ceiling.} Trees are not weight-averageable, so the
        federated ceiling is set by the best \emph{averageable} model. The
        Year-1 instrument is logistic regression at AUROC $0.666$; the
        model-capacity check of Section~\ref{sec:mlp_check} later adds a
        one-hidden-layer MLP that centralizes at $0.670$ and federates to the
        same $0.670$ at $K{=}3$. The $0.007$--$0.011$ gap between the tree
...
magnitudes are moderated by the large cohort and the rank-robustness of AUROC;
the natural further worry---that the linear averageable client additionally
\emph{dampens} them---was tested directly, and dismissed, by the executed
model-capacity check reported next.

\subsection{Model-Capacity Robustness: the MLP Amplification Check (Executed)}
\label{sec:mlp_check}

The churn study above was measured on a class-weighted logistic-regression
client, leaving open whether its modest magnitudes are an artifact of a
saturated linear model. To settle this, the \emph{entire} churn grid---the
three-regime rate sweep, both whole-silo exits, and the count-matched
control---was rerun with two clients on identical partitions and schedules
(five seeds, $K{=}3$, class-weighted FedAvg, 40 rounds; 200 training runs in
total): the original logistic regression, and a one-hidden-layer MLP (64 ReLU
units, He initialization, learning rate $0.05$). The MLP configuration was
selected by a small sweep so that it is a genuinely \emph{stronger} averageable
client, not a handicapped stand-in: it centralizes at AUROC $0.670$, above the
linear ceiling of $0.666$ and inside the published band. Aggregation is
model-agnostic---both clients are encoded as a single flat parameter vector, so
the FedAvg weighting, churn schedules, and harness are unchanged---and the
logistic-regression arm of the rerun reproduces the published churn table
exactly, so the two arms differ only in the client model.

\begin{table}[htbp]
\centering
\caption{Executed MLP amplification check: $\Delta$AUROC vs.\ the same-seed
no-churn baseline under identical partitions and consent schedules, mean
($\pm$\,sd) over five seeds. No condition shows amplification beyond noise.}
\label{tab:mlp_check}
\begin{tabular}{lrr}
\toprule
...
Three findings close the question (Table~\ref{tab:mlp_check}):

\begin{enumerate}
  \item \textbf{No amplification: the churn deltas are not a linear-model
        artifact.} Across all twelve rate-sweep cells the MLP deltas match the
        logistic-regression deltas within one standard deviation
        (amplification ratios $0.2$--$1.6$), and in the \emph{biased} regime---
        the one that carries the mechanism---the ratio is $\approx 0.9$, i.e.\
        marginally \emph{smaller} on the MLP. Silo-level effects are likewise
        unchanged. The caveat is resolved in the direction that
        \emph{strengthens} the result: the cost structure of consent churn is
        model-independent at this capacity.
  \item \textbf{H2 survives the model swap.} On the MLP, the positive-heavy
        whole-silo exit ($-0.064$) still dominates the count-matched random
        control ($-0.051$) at identical headcount, and dwarfs the
        positive-light exit ($-0.004$)---a ${\sim}18\times$ who-leaves gap.
        ``Who leaves matters more than how many leave'' is not a property of
        logistic regression.
  \item \textbf{Why nothing amplifies: the task ceiling, not the model class,
        bounds the deltas.} The MLP lifts the averageable ceiling only
        $0.666 \rightarrow 0.670$ against a tree ceiling of $0.677$ and a
        published band of $0.667$--$0.70$; on a task at its signal ceiling there
        is little headroom for \emph{any} model to lose dramatically more under
        distribution shift. One secondary observation: the MLP's \emph{no-churn}
        baseline is slightly more fragile under severe skew
        ($\alpha{=}0.1$: $0.629$ vs.\ the linear client's $0.640$)---higher
        capacity overfits skewed silos marginally faster.
\end{enumerate}

The utility-cost axis is therefore closed and model-robust: on a real clinical
task at its signal ceiling, per-patient consent churn stays cheap and
distribution-shifting churn stays roughly $3$--$6\times$ more expensive,
regardless of client capacity.
...
aggregation methods over two averageable clients (logistic regression and a
one-hidden-layer MLP behind the same flat-parameter interface), and a library
of consent-churn schedules (transient, permanent-random, permanent-biased,
whole-silo, and the count-matched control), with the five-seed results---%
including the 200-run two-client amplification check---reported in
...
(Section~\ref{sec:executed_baseline}), the three-regime degradation study
with its count-matched isolation test confirming H2
(Table~\ref{tab:churn_results}), and the model-capacity replication showing the
churn deltas are client-independent (Table~\ref{tab:mlp_check}).
\emph{[Remaining results to be completed as experiments conclude.]} The central remaining result is the
...
utility magnitudes are moderated by the large cohort and AUROC's
rank-robustness; the executed MLP replication (Section~\ref{sec:mlp_check})
rules out the linear client as their cause, so the remaining external-validity
question is the Dirichlet partitioning itself, not the model class. Synthetic
data is confined to the systems axis,
...
\section{Timeline of Completed and Remaining Work}
\label{sec:timeline}

Table~\ref{tab:timeline} summarizes the state of the work at the time of this
proposal and the schedule for what remains. The utility-cost axis (RQ2) is
executed and closed; the critical path runs through the systems-cost axis (RQ1)
to the joint frontier (RQ3), with the multi-project matrix (RQ4) layered on the
same infrastructure. Dates are indicative and will be adjusted with the
committee.

\begin{table}[htbp]
\centering
\caption{Dissertation timeline: completed milestones and remaining work.}
\label{tab:timeline}
\begin{tabular}{p{3.2cm}p{9.6cm}}
\toprule
\textbf{Period} & \textbf{Milestone} \\
\midrule
2024 \emph{(done)} & Component papers published: Spark benchmarking
  (Chapter~\ref{chap:spark}), blockchain/IPFS consent layer
  (Chapter~\ref{chap:blockchain}), supply-chain regime shift
  (Chapter~\ref{chap:supplychain}). \\
Spring 2026 \emph{(done)} & Thesis framing finalized (two-axis cost of
  autonomy); primary dataset locked (Diabetes 130); Synthea confined to the
  systems axis. \\
Summer 2026 \emph{(done)} & Utility axis executed: centralized multi-model
  ceiling, five-method federated comparison, three-regime consent-churn study
  with the count-matched isolation test (H2 confirmed), and the MLP
  amplification check (churn deltas shown model-independent,
  Section~\ref{sec:mlp_check}). \\
\midrule
Fall 2026 & Systems-cost axis (RQ1): gas/latency/bandwidth instrumentation on
  the per-round hook across local, permissioned, and public backends, with the
  existing churn schedules as the transaction-traffic generator; Synthea
  scaling to locate the H1 infeasibility threshold. \\
Winter 2026--Spring 2027 & Joint cost--utility frontier (RQ3): both axes swept
  against the shared churn knob; locate the amortization knee. Multi-project
  consent matrix and marginal-cost measurement (RQ4). \\
Spring--Summer 2027 & Robustness replication on real hospital silos (eICU);
...
two robustness dimensions are already closed---the aggregator (five methods
compared, class-weighted FedAvg justified empirically,
Section~\ref{sec:executed_baseline}) and model capacity (the executed MLP
amplification check, Section~\ref{sec:mlp_check})---so the remaining check is
replication on real hospital silos (eICU) in place of Dirichlet partitions.
(3) \textbf{Federated unlearning:} on permanent withdrawal,
